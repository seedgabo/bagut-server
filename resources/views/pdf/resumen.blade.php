...
{!! Form::open(['method' => 'GET', 'class' => 'form-inline']) !!}
    ...
    <div class="form-group">
        {!! Form::submit("Buscar Registros", ['class' => 'btn btn-success']) !!}
    </div>
        ...
        <div class="table table-bordered table-striped display">
			<table id="datatable" class="table table-hover datatable">
				...
				<tbody>
					...
					<tr>
						<td>{{$user->nombre}} {!! $user->getButtonAuditar() !!}</td>
						<td>{{$user->creados}}</td>
						<td>{{$user->responsables}}</td>
						<td>{{$user->comentarios}}</td>
						<td>{{$user->abiertos}}  @if($user->responsables !=0) ({{number_format($user->abiertos/$user->responsables * 100,2)}}%)  @endif</td>
						<td>{{$user->completados}}  @if($user->responsables !=0) ({{number_format($user->completados/$user->responsables * 100,2)}}%)  @endif</td>
						<td>{{$user->vencidos}}  @if($user->responsables !=0) ({{number_format($user->vencidos/$user->responsables * 100,2)}}%)  @endif</td>
						<td>{{$user->logins}}</td>
					</tr>
					@empty
					@endforelse
