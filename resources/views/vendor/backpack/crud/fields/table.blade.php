...
<?php
    ...
    $types = isset($field['types']) ? $field['types'] : [];
...
<div ng-app="backpackTable" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items" class="array-row">
                    ...
                    <td>
                        <input class="form-control input-sm" type="{{$types[$loop->index] or  'text'}}" ng-model="item.{{ $prop }}" min="1">
                    </td>
                    @endforeach
                    <td>
                        <span class="btn btn-sm btn-default sort-handle"><span class="sr-only">{{Lang::get('literales.sort')}} item</span><i class="fa fa-sort" role="presentation" aria-hidden="true"></i></span>
                    </td>
                    <td>
                        <button ng-hide="min > -1 && $index < min" class="btn btn-sm btn-default" type="button" ng-click="removeItem(item);"><span class="sr-only">{{Lang::get('literales.delete')}} item</span><i class="fa fa-trash" role="presentation" aria-hidden="true"></i></button>
                    </td>
                </tr>
        ...
        <div class="array-controls btn-group">
            <button class="btn btn-sm btn-default" type="button" ng-click="addItem()"><i class="fa fa-plus"></i> {{Lang::get('literales.add')}} {{ $item_name }}</button>
        </div>
