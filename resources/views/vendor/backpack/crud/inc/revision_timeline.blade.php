  ...
  <li class="timeline-item-wrap">
    ...
    <div class="timeline-item">
      ...
      @if($history->key == 'created_at' && !$history->old_value)
        <h3 class="timeline-header">{{ $history->userResponsible()->name or 'Un Usuario' }} creó {{ $crud->entity_name }}</h3>
      @else
        <h3 class="timeline-header">{{ $history->userResponsible()->name or 'Un Usuario' }}  Cambió {{ $history->fieldName() }}</h3>
